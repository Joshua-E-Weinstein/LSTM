...
def normalize_data(data_path, n_timesteps, for_periods):
    ...
    data_train = data['1995':'2020']
    data_test = data.values[len(data)-len(data['2021':])-n_timesteps-for_periods+1:]
    ...
    y_train = []
    y_test = []
    for i in range(n_timesteps, data_train_len-for_periods+1):
        X_train.append(data_train_scaled[i-n_timesteps:i, 0:3])
        y_train.append(data_train_scaled[i:i+for_periods, 0])
    for i in range(n_timesteps, data_test_len-for_periods+1):
        X_test.append(data_test_scaled[i-n_timesteps:i, 0:3])
